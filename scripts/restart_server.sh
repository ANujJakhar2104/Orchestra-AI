#!/bin/bash

# Restart Script for OrchestraAI Voice Agents
# Stops any running instances and starts the server fresh

echo "🔄 Restarting OrchestraAI Voice Agents Server..."
echo "================================================"

# Stop any running instances
echo "⏹️  Stopping existing server instances..."
pkill -f "uvicorn app.main:app" || echo "No running instances found"
pkill -f "python.*app/main.py" || true
sleep 2

# Activate virtual environment
if [ -d ".venv" ]; then
    source .venv/bin/activate
elif [ -d "venv" ]; then
    source venv/bin/activate
else
    echo "⚠️  No virtual environment found. Creating one..."
    python3 -m venv .venv
    source .venv/bin/activate
    echo "📦 Installing dependencies..."
    pip install -r requirements.txt
fi

echo "✅ Virtual environment activated"

# Clear Python cache
echo "🧹 Clearing Python cache..."
find . -type d -name "__pycache__" -exec rm -r {} + 2>/dev/null || true
find . -type f -name "*.pyc" -delete 2>/dev/null || true

export PYTHONUNBUFFERED=1

echo ""
echo "🚀 Starting server on http://0.0.0.0:7860"
echo "📊 WebSocket endpoint: ws://0.0.0.0:7860/ws"
echo "🎭 Personas: Aanya · Arjun · Priya · Serena · Rohan · Zara"
echo "================================================"
echo ""
echo "🔍 HYBRID EMOTION MODE — Watch for:"
echo "  🔄 'HYBRID MODE: Processing audio + text'"
echo "  🎯 'HYBRID RESULT: Primary Emotion...'"
echo "  Mismatch detection: sarcasm / polite masking"
echo ""
echo "📝 Logs starting..."
echo "================================================"
echo ""

python3 -u -m uvicorn app.main:app --host 0.0.0.0 --port 7860 --reload 2>&1 | while IFS= read -r line; do
    echo "[$(date '+%H:%M:%S')] $line"
done